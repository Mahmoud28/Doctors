...
namespace Seagull.Core.Data.Repository
{
    public class MedicationRepository : Repository<Medication>, IMedicationRepository
    {
        ...
        public MedicationRepository(LibraryDbContext context,IMapper mapper) : base(context)
        {
            _mapper = mapper;
        }
        ...
        public Medication GetById(int id)
        {
            ...
            var Medication = _context.Set<Medication>().FirstOrDefault();
            return Medication;
        }

        //Get All Medications
        public PagingList<Medication> GetAllMedications(pagination pagination, sort sort, string search, string search_operator, string filter)
        {
            dynamic searchFilter = string.Empty;
            var operater = string.IsNullOrEmpty(search_operator) ? null : JObject.Parse(search_operator);
            IQueryable<Medication> query = _context.Set<Medication>();
                        ...
                        result.ForEach(s =>
                        {
                            ...
                            switch ((string)_filter.Key)
                            {
                                case "":
                                    break;
                                default:
                                    var tempQuery = _context.Set<Medication>();
                                    query = count == 0 ? query.Where<Medication>(
                                        (object)_filter.Key, (object)s,
                                        (WhereOperation)StringEnum.Parse(typeof(WhereOperation), op)) :
                                        query.Concat<Medication>(tempQuery.Where<Medication>(
                                        (object)_filter.Key, (object)s,
                                        (WhereOperation)StringEnum.Parse(typeof(WhereOperation), op)));
                                    count = count + 1;
                                    break;
            ...
            query = query.OrderBy<Medication>(sort.predicate, !sort.reverse ? "asc" : "");


            return new PagingList<Medication>(query, pagination.start / 10, pagination.Count == 0 ? 10 : pagination.Count);
        }
