...
{
    public class MedicationFailedRepository : Repository<MedicationFailed>, IMedicationFailedRepository
    {
        private readonly IMapper _mapper;
        public MedicationFailedRepository(LibraryDbContext context,IMapper mapper) : base(context)
        {
            _mapper = mapper;
        }

        public MedicationFailed GetById(int id)
        {

            var MedicationFailed = _context.Set<MedicationFailed>().Where(d => d.Id == id).FirstOrDefault();
            return MedicationFailed;
        }

     


        //Get All MedicationFaileds
        public PagingList<MedicationFailed> GetAllMedicationFaileds(pagination pagination, sort sort, string search, string search_operator, string filter)
        {
            dynamic searchFilter = string.Empty;
            var operater = string.IsNullOrEmpty(search_operator) ? null : JObject.Parse(search_operator);
            IQueryable<MedicationFailed> query = _context.Set<MedicationFailed>();
            ...
            if (!string.IsNullOrEmpty(filter) && filter.Length > 2)
            {
                ...
                foreach (var _filter in searchFilter)
                {
                        ...
                        result.ForEach(s =>
                        {
                            ...
                            switch ((string)_filter.Key)
                            {
                                case "":
                                    break;
                                default:
                                    var tempQuery = _context.Set<MedicationFailed>();
                                    query = count == 0 ? query.Where<MedicationFailed>(
                                        (object)_filter.Key, (object)s,
                                        (WhereOperation)StringEnum.Parse(typeof(WhereOperation), op)) :
                                        query.Concat<MedicationFailed>(tempQuery.Where<MedicationFailed>(
                                        (object)_filter.Key, (object)s,
                                        (WhereOperation)StringEnum.Parse(typeof(WhereOperation), op)));
                                    count = count + 1;
                                    break;
                    ...
                    else if (!Object.ReferenceEquals(null, _filter.Value) && !String.IsNullOrEmpty(_filter.Value.ToString()))
                    {
                        ...
                        query = query.Where<MedicationFailed>(
            ...
            query = query.OrderBy<MedicationFailed>(sort.predicate, !sort.reverse ? "asc" : "");


            return new PagingList<MedicationFailed>(query, pagination.start / 10, pagination.Count == 0 ? 10 : pagination.Count);
        }
