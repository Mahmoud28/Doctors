...
namespace Seagull.Core.Data.Repository
{
    public class PharmacyRepository : Repository<Pharmacy>, IPharmacyRepository
    {
        ...
        public PharmacyRepository(LibraryDbContext context,IMapper mapper) : base(context)
        {
            _mapper = mapper;
        }
        ...
        public Pharmacy GetById(int id)
        {
            ...
            var Pharmacy = _context.Set<Pharmacy>().Where(d => d.Id == id).FirstOrDefault();
            return Pharmacy;
        }
        ...
        //Get All Pharmacys
        public PagingList<Pharmacy> GetAllPharmacys(pagination pagination, sort sort, string search, string search_operator, string filter)
        {
            dynamic searchFilter = string.Empty;
            var operater = string.IsNullOrEmpty(search_operator) ? null : JObject.Parse(search_operator);
            IQueryable<Pharmacy> query = _context.Set<Pharmacy>();
                        ...
                        result.ForEach(s =>
                        {
                            ...
                            switch ((string)_filter.Key)
                            {
                                case "":
                                    break;
                                default:
                                    var tempQuery = _context.Set<Pharmacy>();
                                    query = count == 0 ? query.Where<Pharmacy>(
                                        (object)_filter.Key, (object)s,
                                        (WhereOperation)StringEnum.Parse(typeof(WhereOperation), op)) :
                                        query.Concat<Pharmacy>(tempQuery.Where<Pharmacy>(
                                        (object)_filter.Key, (object)s,
                                        (WhereOperation)StringEnum.Parse(typeof(WhereOperation), op)));
                                    count = count + 1;
                                    break;
            ...
            query = query.OrderBy<Pharmacy>(sort.predicate, !sort.reverse ? "asc" : "");


            return new PagingList<Pharmacy>(query, pagination.start / 10, pagination.Count == 0 ? 10 : pagination.Count);
        }
