...
namespace Seagull.Core.Data.Repository
{
    public class VisitTypeRepository : Repository<VisitType>, IVisitTypeRepository
    {
        ...
        public VisitTypeRepository(LibraryDbContext context,IMapper mapper) : base(context)
        {
            _mapper = mapper;
        }
        ...
        {
            var VisitType = _context.Set<VisitType>().Where(d => d.Id == id).FirstOrDefault();
            return VisitType;
        }
        ...
        //Get All VisitTypes
        public PagingList<VisitType> GetAllVisitTypes(pagination pagination, sort sort, string search, string search_operator, string filter)
        {
            dynamic searchFilter = string.Empty;
            var operater = string.IsNullOrEmpty(search_operator) ? null : JObject.Parse(search_operator);
            IQueryable<VisitType> query = _context.Set<VisitType>();
                        ...
                        result.ForEach(s =>
                        {
                            ...
                            switch ((string)_filter.Key)
                            {
                                case "":
                                    break;
                                default:
                                    var tempQuery = _context.Set<VisitType>();
                                    query = count == 0 ? query.Where<VisitType>(
                                        (object)_filter.Key, (object)s,
                                        (WhereOperation)StringEnum.Parse(typeof(WhereOperation), op)) :
                                        query.Concat<VisitType>(tempQuery.Where<VisitType>(
                                        (object)_filter.Key, (object)s,
                                        (WhereOperation)StringEnum.Parse(typeof(WhereOperation), op)));
                                    count = count + 1;
                                    break;
            ...
            query = query.OrderBy<VisitType>(sort.predicate, !sort.reverse ? "asc" : "");


            return new PagingList<VisitType>(query, pagination.start / 10, pagination.Count == 0 ? 10 : pagination.Count);
        }
